...
test("workflow run writes workspace compatibility metadata", () => {
  ...
  assert.equal(metadata.min_read_runtime_version, "0.1.13");
  assert.equal(metadata.min_write_runtime_version, "0.1.13");
  assert.equal(metadata.last_writer_runtime_version, "0.1.13");
...
test("workspace compatibility diagnostics refuse unsupported reads and newer write runtimes", () => {
  ...
  assert.equal(cliCompatibility.current_runtime_version, "0.1.13");
