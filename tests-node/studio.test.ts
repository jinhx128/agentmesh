...
test("Studio server exposes workspace compatibility diagnostics", async () => {
  ...
  assert.equal(compatibility.current_runtime_version, "0.3.3");
...
test("Studio server exposes AgentMesh update diagnostics", async () => {
  ...
  await withReleaseServer(releasePayload("0.3.4"), async (releaseUrl) => {
    ...
    try {
      ...
      assert.equal(update.current_version, "0.3.3");
      assert.equal(update.latest_version, "0.3.4");
      ...
      assert.equal(update.desktop.asset_url, "https://example.invalid/AgentMesh_0.3.4_aarch64.dmg");
    } finally {
