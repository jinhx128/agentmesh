...
test("Studio server exposes workspace compatibility diagnostics", async () => {
  ...
  assert.equal(compatibility.current_runtime_version, "0.1.10");
...
test("Studio server exposes AgentMesh update diagnostics", async () => {
  ...
  await withReleaseServer(releasePayload("0.1.11"), async (releaseUrl) => {
    ...
    try {
      ...
      assert.equal(update.current_version, "0.1.10");
      assert.equal(update.latest_version, "0.1.11");
      ...
      assert.equal(update.desktop.asset_url, "https://example.invalid/AgentMesh_0.1.11_aarch64.dmg");
    } finally {
