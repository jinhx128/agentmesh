...
test("version commands report the installed AgentMesh version", () => {
  ...
  assert.equal(payload.current_version, "0.1.3");
...
test("update check reports newer CLI and Desktop release assets", async () => {
  ...
  await withReleaseServer(releasePayload("0.1.4"), async (releaseUrl) => {
    ...
    assert.equal(payload.schema_version, 1);
    assert.equal(payload.current_version, "0.1.3");
    assert.equal(payload.latest_version, "0.1.4");
    assert.equal(payload.update_available, true);
    assert.equal(payload.cli.status, "update_available");
    assert.equal(payload.cli.asset_name, "agentmesh-0.1.4.tgz");
    assert.equal(payload.cli.asset_url, "https://example.invalid/agentmesh-0.1.4.tgz");
    ...
    assert.equal(payload.desktop.asset_name, "AgentMesh_0.1.4_aarch64.dmg");
    assert.equal(payload.desktop.asset_url, "https://example.invalid/AgentMesh_0.1.4_aarch64.dmg");
  });
});
...
test("update check falls back to the release page when the API is rate limited", async () => {
  const workspace = makeWorkspace();
  test.after(() => rmSync(workspace, { recursive: true, force: true }));
  await withReleasePageFallbackServer("0.1.4", async ({ apiReleaseUrl, webReleaseUrl }) => {
    const check = await runCliAsync(workspace, ["update", "check", "--json"], {
    ...
    assert.equal(payload.latest_version, "0.1.4");
    assert.equal(payload.update_available, true);
    assert.match(payload.release_url, /\/releases\/tag\/v0\.1\.4$/);
    assert.equal(payload.cli.status, "update_available");
    assert.equal(payload.cli.asset_name, "agentmesh-0.1.4.tgz");
    assert.match(payload.cli.asset_url ?? "", /\/releases\/download\/v0\.1\.4\/agentmesh-0\.1\.4\.tgz$/);
    assert.equal(payload.desktop.status, "manual_update_available");
    assert.equal(payload.desktop.asset_name, "AgentMesh_0.1.4_aarch64.dmg");
    assert.match(payload.desktop.asset_url ?? "", /\/releases\/download\/v0\.1\.4\/AgentMesh_0\.1\.4_aarch64\.dmg$/);
  });
});
...
test("update install dry-run reports CLI command and Desktop manual download", async () => {
  ...
  await withReleaseServer(releasePayload("0.1.4"), async (releaseUrl) => {
    ...
    assert.equal(desktopPayload.asset_url, "https://example.invalid/AgentMesh_0.1.4_aarch64.dmg");
...
test("update install dry-run reports current when the release is not newer", async () => {
  const workspace = makeWorkspace();
  test.after(() => rmSync(workspace, { recursive: true, force: true }));
  await withReleaseServer(releasePayload("0.1.3"), async (releaseUrl) => {
    const cli = await runCliAsync(workspace, ["update", "install", "--target", "cli", "--dry-run", "--json"], {
      AGENTMESH_UPDATE_RELEASE_URL: releaseUrl,
    });
    assert.equal(cli.status, 0, cli.stderr);
