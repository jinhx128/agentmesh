...
test("version commands report the installed AgentMesh version", () => {
  ...
  assert.equal(shortVersion.stdout.trim(), "0.3.3");
  ...
  assert.equal(payload.current_version, "0.3.3");
...
test("update check reports newer CLI and Desktop release assets", async () => {
  const workspace = makeWorkspace();
  test.after(() => rmSync(workspace, { recursive: true, force: true }));
  await withReleaseServer(releasePayload("0.3.4"), async (releaseUrl) => {
    ...
    assert.equal(payload.schema_version, 1);
    assert.equal(payload.current_version, "0.3.3");
    assert.equal(payload.latest_version, "0.3.4");
    assert.equal(payload.update_available, true);
    assert.equal(payload.cli.status, "update_available");
    assert.equal(payload.cli.asset_name, "agentmesh-0.3.4.tgz");
    assert.equal(payload.cli.asset_url, "https://example.invalid/agentmesh-0.3.4.tgz");
    assert.deepEqual(payload.cli.install_command, [
      "npm",
      "install",
      "-g",
      "https://example.invalid/agentmesh-0.3.4.tgz",
    ]);
    assert.equal(payload.desktop.status, "manual_update_available");
    assert.equal(payload.desktop.asset_name, "AgentMesh_0.3.4_aarch64.dmg");
    assert.equal(payload.desktop.asset_url, "https://example.invalid/AgentMesh_0.3.4_aarch64.dmg");
  });
});
...
test("update check falls back to the release page when the API is rate limited", async () => {
  const workspace = makeWorkspace();
  test.after(() => rmSync(workspace, { recursive: true, force: true }));
  await withReleasePageFallbackServer("0.3.4", async ({ apiReleaseUrl, webReleaseUrl }) => {
    const check = await runCliAsync(workspace, ["update", "check", "--json"], {
    ...
    assert.equal(payload.latest_version, "0.3.4");
    assert.equal(payload.update_available, true);
    assert.match(payload.release_url, /\/releases\/tag\/v0\.3\.4$/);
    assert.equal(payload.cli.status, "update_available");
    assert.equal(payload.cli.asset_name, "agentmesh-0.3.4.tgz");
    assert.match(payload.cli.asset_url ?? "", /\/releases\/download\/v0\.3\.4\/agentmesh-0\.3\.4\.tgz$/);
    assert.equal(payload.desktop.status, "manual_update_available");
    assert.equal(payload.desktop.asset_name, "AgentMesh_0.3.4_aarch64.dmg");
    assert.match(payload.desktop.asset_url ?? "", /\/releases\/download\/v0\.3\.4\/AgentMesh_0\.3\.4_aarch64\.dmg$/);
  });
});
...
test("update install dry-run reports CLI command and Desktop manual download", async () => {
  ...
  await withReleaseServer(releasePayload("0.3.4"), async (releaseUrl) => {
    ...
    assert.equal(desktopPayload.asset_url, "https://example.invalid/AgentMesh_0.3.4_aarch64.dmg");
