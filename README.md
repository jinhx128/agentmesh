...
```text
https://github.com/jinhx128/agentmesh/releases/tag/v0.1.14
```

Release 资产：

- `agentmesh-0.1.14.tgz`：CLI npm tarball，可用 `npm install -g` 安装。
- `AgentMesh_0.1.14_aarch64.dmg`：macOS Apple Silicon Desktop Studio，未签名、未 notarize。
- `AgentMesh_0.1.14_aarch64.app.tar.gz`：Tauri 应用内更新归档。
- `AgentMesh_0.1.14_aarch64.app.tar.gz.sig`：更新归档签名。
- `latest.json`：stable updater 元数据，使用不可变 `v0.1.14` 资产 URL。
- `agentmesh-skill-0.1.14.md`：单独下载的 AgentMesh Skill markdown。
- `SHA256SUMS`：发布文件校验值。

v0.1.14 主要增加跨 Codex、Cursor、Claude Code、Antigravity 和 OpenCode 入口的
...
Studio 同时更新了统一活动导航、分组五项预览与展开、状态标签、活动删除、CLI 自动检测/安装/
升级入口，以及 Desktop 自动检查更新偏好。CLI 与 Desktop 继续是两个独立安装渠道。
...
```bash
npm install -g https://github.com/jinhx128/agentmesh/releases/download/v0.1.14/agentmesh-0.1.14.tgz
agentmesh --help
agentmesh --version
...
```bash
npm install -g ./agentmesh-0.1.14.tgz
agentmesh --help
agentmesh --version
agentmesh doctor --json
```
...
打开 DMG，把 `AgentMesh.app` 拖到 Applications。因为 v0.1.14 的 DMG 未签名且未 notarize，macOS
...
Release 里的 `agentmesh-skill-0.1.14.md` 是同一份可单独下载的 markdown。手动安装时，
...
- `current packet schema is active`：v0.1.14 只按当前 packet schema 创建和推进 run。
