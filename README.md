...
```text
https://github.com/jinhx128/agentmesh/releases/tag/v0.1.7
```

Release 资产：

- `agentmesh-0.1.7.tgz`：CLI npm tarball，可用 `npm install -g` 安装。
- `AgentMesh_0.1.7_aarch64.dmg`：macOS Apple Silicon Desktop Studio，未签名。
- `agentmesh-skill-0.1.7.md`：单独下载的 AgentMesh Skill markdown。
- `SHA256SUMS`：发布文件校验值。
...
```bash
npm install -g https://github.com/jinhx128/agentmesh/releases/download/v0.1.7/agentmesh-0.1.7.tgz
agentmesh --help
agentmesh --version
...
```bash
npm install -g ./agentmesh-0.1.7.tgz
agentmesh --help
agentmesh --version
agentmesh doctor --json
```
...
```text
AgentMesh_0.1.7_aarch64.dmg
```

打开 DMG，把 `AgentMesh.app` 拖到 Applications。因为 v0.1.7 的 DMG 未签名，macOS
...
Release 里的 `agentmesh-skill-0.1.7.md` 是同一份可单独下载的 markdown。手动安装时，
...
- `current packet schema is active`：v0.1.7 只按当前 packet schema 创建和推进 run。
