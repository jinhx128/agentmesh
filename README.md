...
```text
https://github.com/jinhx128/agentmesh/releases/tag/v0.3.2
```

Release 资产：

- `agentmesh-0.3.2.tgz`：CLI npm tarball，可用 `npm install -g` 安装。
- `AgentMesh_0.3.2_aarch64.dmg`：macOS Apple Silicon Desktop Studio，未签名、未 notarize。
- `AgentMesh_0.3.2_aarch64.app.tar.gz`：Tauri 应用内更新归档。
- `AgentMesh_0.3.2_aarch64.app.tar.gz.sig`：更新归档签名。
- `latest.json`：stable updater 元数据，使用不可变 `v0.3.2` 资产 URL。
- `agentmesh-skill-0.3.2.md`：单独下载的 AgentMesh Skill markdown。
- `SHA256SUMS`：发布文件校验值。

v0.3.2 修复 macOS 代理例外列表转发给 agent 子进程时的写法不兼容问题：
`*.example.com` 这类 macOS glob 写法和 `<local>` token 在部分 agent CLI 中匹配不到目标主机，
本应绕过代理的内网端点仍被转发给代理，agent 调用表现为连接反复重试直至超时。
现在统一改写为各 CLI 都能识别的裸域名形式。

本版本不改变数据格式与命令行接口，也不改变 Packet schema。
从 v0.3.1 升级可直接覆盖安装。CLI 与 Desktop 继续是两个独立安装渠道。
...
```bash
npm install -g https://github.com/jinhx128/agentmesh/releases/download/v0.3.2/agentmesh-0.3.2.tgz
agentmesh --help
agentmesh --version
...
```bash
npm install -g ./agentmesh-0.3.2.tgz
agentmesh --help
agentmesh --version
agentmesh doctor --json
```
...
```text
AgentMesh_0.3.2_aarch64.dmg
```

打开 DMG，把 `AgentMesh.app` 拖到 Applications。因为 v0.3.2 的 DMG 未签名且未 notarize，macOS
...
Release 里的 `agentmesh-skill-0.3.2.md` 是同一份可单独下载的 markdown。Skill 安装到用户
...
- `current packet schema is active`：v0.3.2 只按当前 packet schema 创建和推进 run。
