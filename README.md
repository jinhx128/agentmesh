...
```text
https://github.com/jinhx128/agentmesh/releases/tag/v0.2.1
```

Release 资产：

- `agentmesh-0.2.1.tgz`：CLI npm tarball，可用 `npm install -g` 安装。
- `AgentMesh_0.2.1_aarch64.dmg`：macOS Apple Silicon Desktop Studio，未签名、未 notarize。
- `AgentMesh_0.2.1_aarch64.app.tar.gz`：Tauri 应用内更新归档。
- `AgentMesh_0.2.1_aarch64.app.tar.gz.sig`：更新归档签名。
- `latest.json`：stable updater 元数据，使用不可变 `v0.2.1` 资产 URL。
- `agentmesh-skill-0.2.1.md`：单独下载的 AgentMesh Skill markdown。
- `SHA256SUMS`：发布文件校验值。

v0.2.1 修复使用第三方 Provider/API Key 添加 Codex Agent 时被官方登录态阻断的问题。
当 `codex login status` 不可用时，AgentMesh 会用该 Agent 的 provider、model 和 reasoning
配置发起一次临时、只读的最小模型探测；第三方凭据有效即可完成添加，不要求额外登录官方账号。
探测会区分无效 Key、模型不可用、网络失败、空响应和超时，Codex 默认探测超时为 60 秒。
...
```bash
npm install -g https://github.com/jinhx128/agentmesh/releases/download/v0.2.1/agentmesh-0.2.1.tgz
agentmesh --help
agentmesh --version
...
```bash
npm install -g ./agentmesh-0.2.1.tgz
agentmesh --help
agentmesh --version
agentmesh doctor --json
```
...
打开 DMG，把 `AgentMesh.app` 拖到 Applications。因为 v0.2.1 的 DMG 未签名且未 notarize，macOS
...
- `current packet schema is active`：v0.2.1 只按当前 packet schema 创建和推进 run。
