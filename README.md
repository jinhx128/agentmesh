...
```text
https://github.com/jinhx128/agentmesh/releases/tag/v0.3.3
```

Release 资产：

- `agentmesh-0.3.3.tgz`：CLI npm tarball，可用 `npm install -g` 安装。
- `AgentMesh_0.3.3_aarch64.dmg`：macOS Apple Silicon Desktop Studio，未签名、未 notarize。
- `AgentMesh_0.3.3_aarch64.app.tar.gz`：Tauri 应用内更新归档。
- `AgentMesh_0.3.3_aarch64.app.tar.gz.sig`：更新归档签名。
- `latest.json`：stable updater 元数据，使用不可变 `v0.3.3` 资产 URL。
- `agentmesh-skill-0.3.3.md`：单独下载的 AgentMesh Skill markdown。
- `SHA256SUMS`：发布文件校验值。

v0.3.3 把 v0.3.2 的代理例外改写补齐到桌面端。桌面端的 Rust 侧另有一份独立的
`scutil --proxy` 解析，v0.3.2 只修了 runtime 的 TypeScript 路径，Desktop Studio 转发给
sidecar 的 `NO_PROXY` 仍是 `*.example.com` 与 `<local>` 原样写法，导致桌面端内的
更新检查与 agent 派发访问例外列表内端点时仍会错误地走代理。

本版本不改变数据格式与命令行接口，也不改变 Packet schema。
从 v0.3.2 升级可直接覆盖安装。CLI 与 Desktop 继续是两个独立安装渠道。
...
```bash
npm install -g https://github.com/jinhx128/agentmesh/releases/download/v0.3.3/agentmesh-0.3.3.tgz
agentmesh --help
agentmesh --version
...
```bash
npm install -g ./agentmesh-0.3.3.tgz
agentmesh --help
agentmesh --version
agentmesh doctor --json
```
...
```text
AgentMesh_0.3.3_aarch64.dmg
```

打开 DMG，把 `AgentMesh.app` 拖到 Applications。因为 v0.3.3 的 DMG 未签名且未 notarize，macOS
...
Release 里的 `agentmesh-skill-0.3.3.md` 是同一份可单独下载的 markdown。Skill 安装到用户
...
- `current packet schema is active`：v0.3.3 只按当前 packet schema 创建和推进 run。
