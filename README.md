...
```text
https://github.com/jinhx128/agentmesh/releases/tag/v0.1.1
```

Release 资产：

- `agentmesh-0.1.1.tgz`：CLI npm tarball，可用 `npm install -g` 安装。
- `AgentMesh_0.1.1_aarch64.dmg`：macOS Apple Silicon Desktop Studio，未签名。
- `agentmesh-skill-0.1.1.md`：单独下载的 AgentMesh Skill markdown。
- `SHA256SUMS`：发布文件校验值。

根 `package.json` 仍是 `private: true`，所以公共 npm registry 上没有
`npm install -g agentmesh`。推荐从 GitHub Release 的 tarball 安装 CLI。
...
```bash
npm install -g https://github.com/jinhx128/agentmesh/releases/download/v0.1.1/agentmesh-0.1.1.tgz
agentmesh --help
...
```bash
npm install -g ./agentmesh-0.1.1.tgz
agentmesh --help
agentmesh doctor --json
```
...
```text
AgentMesh_0.1.1_aarch64.dmg
```

打开 DMG，把 `AgentMesh.app` 拖到 Applications。因为 v0.1.1 的 DMG 未签名，macOS
...
启动；未指定时不会把 macOS app 的 `/` 工作目录当作 workspace。
...
Release 里的 `agentmesh-skill-0.1.1.md` 是同一份可单独下载的 markdown。手动安装时，
...
| `antigravity-cli` | `antigravity` | `agy` | 模型由 Antigravity CLI 内部当前选择控制，AgentMesh 使用 `current` | 不传 `-m`，调用形式是 `agy -p <prompt>` |
...
agentmesh agents add \
  --adapter antigravity \
  --model current \
  --label "Antigravity Current" \
  --reasoning-effort none
```

Antigravity 的模型列表来自本机 `agy` 的可用模型和本地用户数据。AgentMesh 不写死
Gemini、Claude 或其他模型名；发现到什么就展示什么。真正运行时不向 `agy` 传模型参数，
而是使用 Antigravity CLI 当前会话内选择的模型。
...
- `current packet schema is active`：v0.1.1 只按当前 packet schema 创建和推进 run。
