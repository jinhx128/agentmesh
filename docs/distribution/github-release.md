...
- Release assets 至少包含：
  ...
  - `SHA256SUMS`
- `npm run publish:github` 或 `npm run release:github:verify` 通过。
...
- `0.1.10` 到首个 updater-enabled 版本 `0.1.11` 需要手动替换一次 DMG，安装
  `0.1.11` 后的后续版本可在应用内更新。
...
`npm run publish:github` 会在创建或补传后自动调用 verify。单独验证时必须检查到
Release 不是 draft / prerelease，全部版本化资产和 `latest.json` 都已上传。底层 `npm run
...
- `dist-release/AgentMesh_<version>_aarch64.app.tar.gz`
- `dist-release/AgentMesh_<version>_aarch64.app.tar.gz.sig`
