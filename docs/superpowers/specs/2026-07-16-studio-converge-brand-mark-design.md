...
`StudioBrandMark.tsx` 通过 `new URL("...agentmesh.svg?no-inline", import.meta.url).href` 引用该 SVG，并渲染为装饰性 `<img>`。该形式同时兼容 Node 静态渲染测试与 Vite asset 转换；`?no-inline` 阻止小型 SVG 被写入 JavaScript data URL。Vite dev server 只额外允许读取 canonical icon 所在目录；production build 将 SVG 作为带 hash 的 frontend asset 输出，Tauri 沿用现有 frontend bundle 打包流程。
...
- Node contract 断言 `StudioBrandMark` 通过 `new URL("...agentmesh.svg?no-inline", import.meta.url)` 引用 canonical asset，包含装饰性 `<img>`，且旧 core/track DOM 与 CSS 已删除。
