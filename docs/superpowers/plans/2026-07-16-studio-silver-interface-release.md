# AgentMesh Studio 银灰界面、0.1.11 与 0.1.12 热修发布实施计划

> 状态（2026-07-16）：已合并。`0.1.11` 与 `0.1.12` 热修代码保留为历史证据；尚未完成的 `0.1.12` 资产、发布和真机升级迁入 `2026-07-16-studio-activity-and-v012-release.md`。本文件不再作为当前发布事实源。
...
- [x] ~~P2.1 确认版本并同步版本源、文档和 release notes。~~
...
- [x] **Step 2: 先改 current-version contract 并验证 RED**
...
- [x] **Step 3: 同步 canonical version sources**
...
- [x] **Step 4: 同步 README、分发文档和正式 release notes**
...
进度记录：状态 `completed`；完成时间 `2026-07-16 10:59 CST`。远端核验事实为 npm latest `0.1.10`、GitHub latest `v0.1.10`、最高 tag `v0.1.10`，故唯一目标锁定为 `0.1.11`。RED 由 package/workspace dependencies、CLI version、Skill metadata 和 update current version 多项 `actual 0.1.10 !== expected 0.1.11` 证明；完整 build 后版本/Skill/update/Studio/分发聚焦测试 155/155 GREEN。root + 9 workspace、内部 exact dependencies、SDK/MCP、Tauri/Cargo、metadata examples、README、distribution docs 和 release notes 已同步；旧 `0.1.10` 只保留在首次 updater 迁移说明及测试的安装/更新/prerelease 场景。首次全量测试 550/551，唯一失败为 owner-lock 测试显式 `runtimeVersion: 0.1.10` 与新 workspace metadata 不兼容，最小同步夹具后聚焦 1/1、最终全量 551/551。`npm audit` 发现 Vite/Hono/qs 的 2 high + 1 moderate 可修复漏洞，已将 Vite range 提升为 `^8.0.16` 并把 lockfile 更新到 Vite `8.1.4`、Hono `4.12.30`、qs `6.15.3`；最终 audit 为 0 vulnerabilities，复跑 `npm test` 551/551、Desktop package `ok: true`、随后 `cargo check` 编译 `agentmesh-studio-desktop v0.1.11` 通过。changelog 已同步；commit 见本 slice 收尾提交；下一步 `P2.2 Step 1`。
...
- [x] **Step 3: 处理 findings 并锁定发布 commit**
...
`P2.3 Step 3` 完成时间 `2026-07-16 12:53 CST`。修正提交为 `c2246a5`；新资产为 npm tgz 504,449 bytes、DMG 35,901,983 bytes、updater archive 36,212,177 bytes、signature 408 bytes、`latest.json` 698 bytes、Skill 12,107 bytes、`SHA256SUMS` 638 bytes。DMG VALID、所有 checksum OK、目录恰好 7 项、metadata/signature/immutable URL 一致，且新 archive 再次通过 Tauri 内嵌公钥真实验签。controller findings 已分为 accepted/rejected/needs decision，`needs decision` 为无；刷新后的 release summary 报 review outputs 与 classified findings 均 present、无 missing evidence。`decision.md` 仅含一个有效 verdict，run 最终状态为 `decide_completed`，`release_verdict.value=ready`、diagnostic `ok`。下一步 `P2.4 Step 1`。

### P2.4 推送 main、打 tag、发布 npm 与 GitHub 资产

> `2026-07-16 14:30 CST` 状态校准：`v0.1.11` tag 与含 7 项资产的 GitHub Release 已发布，npm 因登录过期与 Security Key 交互中断而未发布；本机 Desktop 已到 `0.1.11`。随后发现 Finder/Dock 无参数启动会同步递归扫描 Home，可能长期停在启动页，因此 P2.4 中未完成的 npm 发布与 P2.5 真机收尾由下方 `P3 / 0.1.12` 取代。禁止补发 npm `0.1.11`，也禁止覆盖历史 `v0.1.11` tag/资产。
...
### P3. AgentMesh 0.1.12 Desktop 启动热修与最终发布

- [ ] **P3 阶段完成门禁（仅在 npm、GitHub、CLI、Desktop 与发布后证据全部完成后勾选）**
- 阶段目标：修复打包 Desktop 无参数启动阻塞，发布唯一后继版本 `0.1.12`，并把本机 CLI/Desktop 统一升级到该版本。
- 发布边界：保留历史 `v0.1.11`；npm 只发布 `0.1.12`；签名凭据只从仓库外文件和 Keychain 注入，不打印。

#### P3.1 热修、版本同步、三模型审查与提交

- [x] **Step 1: 修复无参数默认工作区解析**

Finder、Dock 或 `open` 使 `cwd` 为文件系统根且未显式传 workspace 时，只读取 `~/.config/agentmesh/workspaces.json`；选择最近启用且路径存在的工作区。删除同步递归 Home 扫描；registry 缺失、损坏、schema/条目无效、仅 disabled 或路径已删除时回退 Home。Desktop 保留本地只读 parser 以遵守 Desktop → App Server package boundary，排序与 runtime 的活动时间、label、path tie-break 一致。

- [x] **Step 2: 同步 `0.1.12` 版本与文档**

root + 9 workspace、内部 exact dependencies、SDK fallback、MCP client、Tauri/Cargo、stable/beta metadata examples、当前版本测试、README 与 `v0.1.12` release notes 均同步；测试中的 `0.1.13` 只表达新版本可用场景，Cargo `winapi-util 0.1.11` 为第三方依赖。

- [x] **Step 3: 完成 AgentMesh 三模型 review 并处理 findings**

Review run `v012-noargs-hotfix-review-20260716` 状态为 `decide_completed`；Cursor Composer 2.5、OpenCode GLM 5.2、Claude Code Opus 4.8 High 三个 lane 均 exit 0。已接受并处理 registry 排序一致性、损坏/无效/disabled/删除路径回退测试、README 与升级说明 findings；`agentmesh workspaces register` 建议经 CLI help 校准为实际命令 `agentmesh workspaces add`。最终无未解决 Must/Should，无需用户决策。

- [x] **Step 4: 同步 changelog、完整验证并提交热修**

验证 `npm test`、Desktop dev package、Cargo check/test、`npm audit --json` 与 `git diff --check` 后，只提交剩余 hotfix、版本、README、release notes、changelog 和本计划，commit message 使用 `修复：发布 0.1.12 Desktop 启动热修`。

进度记录：`npm test` 553/553、Desktop dev package `ok: true`、`cargo check`、Rust test 1/1、`npm audit --json` 0 vulnerabilities 与 `git diff --check` 均通过；31 个 hotfix/version/docs/test 文件已作为独立 `修复：发布 0.1.12 Desktop 启动热修` commit 提交。下一步 `P3.2 Step 1`。

#### P3.2 生成并验证 `0.1.12` 签名产物

- [ ] **Step 1: 从仓库外 updater key 与 Keychain 安全加载密码，执行 `npm run release:assets`**
- [ ] **Step 2: 验证目录恰好 7 项、DMG VALID、checksums、metadata/signature 与不可变 `v0.1.12` URL**
- [ ] **Step 3: 记录资产名称、字节数与门禁证据；生成物保持 ignored**

#### P3.3 集成、tag、GitHub Release 与 npm 发布

- [ ] **Step 1: 核对 feature/root worktree，fetch 后 fast-forward main，推送 feature/main**
- [ ] **Step 2: 在唯一 release commit 创建并推送 annotated `v0.1.12` tag**
- [ ] **Step 3: 发布并远端验证 7 项 GitHub Release 资产与 stable `latest.json`**
- [ ] **Step 4: 发布并验证 npm `@jinhx128/agentmesh@0.1.12` 与 `latest` dist-tag**

npm 若要求浏览器/Security Key 交互，等待用户完成登录后只继续同一 `0.1.12` 发布，不降级到其他 registry、不打印 OTP、不把未发布状态写成成功。

#### P3.4 本机 CLI/Desktop 升级与最终证据

- [ ] **Step 1: 安装公共 npm `0.1.12`，验证 PATH、`agentmesh --version` 与 update current**
- [ ] **Step 2: 退出 AgentMesh，验证 DMG 后替换 `/Applications/AgentMesh.app` 并无参数启动**
- [ ] **Step 3: 验证 App/sidecar `0.1.12`、最近 registry 工作区、Settings/About updater current 与退出清理**
- [ ] **Step 4: 写发布后 changelog/计划证据，提交并推送 feature/main；聚合唯一 ready 结论**
...
- 本文件是历史发布记录；当前唯一事实源为 `2026-07-16-studio-activity-and-v012-release.md`，下方维护规则不再驱动执行。
...
- 原下一步 `P3.2 Step 1` 已迁入 `2026-07-16-studio-activity-and-v012-release.md`，不得从本文件继续执行。
